...
#################################################################################
# Read in the UPF file for the Reference Design
#################################################################################
if {$UPF_SUPPLEMENTAL_FILE != ""} {

	## For golden UPF flow
        if { [file exists [which $UPF_FILE]] && [file exists [which $UPF_SUPPLEMENTAL_FILE] ]} {
		load_upf -r $UPF_FILE -target dc_pg_netlist -strict_check false -supplemental $UPF_SUPPLEMENTAL_FILE 
        } else {
          	puts "RM-error: UPF_FILE $UPF_FILE or UPF_SUPPLEMENTAL_FILE $UPF_SUPPLEMENTAL_FILE is not found or not defined."
          	puts "RM-info: For golden UPF designs, fm.tcl requires one golden UPF file and supplemental UPF file. Exiting"
		exit
	}
...
# constrained to their ON state.  For ICC2-RM,
