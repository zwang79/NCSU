 ...
 ( tlm::tlm_generic_payload &gp, sc_core::sc_time &delay )
{
  ...
  //sc_core::sc_time(sec,sc_core::SC_NS);
  sc_core::sc_time mem_delay(10,sc_core::SC_NS);

  accu_length += gp.get_data_length();
  ...
  //time_record = sc_core::sc_time_stamp();
  sec = sc_core::sc_time_stamp().to_seconds();
  ave = accu_length/sec;
  //cout << "************* "<< sc_object::name() << "##" <<accu_length << " *************" << endl;
  //cout << "************* "<< sec << " *************" << endl;
  //cout << "mem.ave = "<< ave << endl;
  return;     
}
