...
if { $error } {
  puts "Usage: gen_clock_tree_matrix_icc2.tcl"
...
proc sproc_gather_build_data { build_label build_dir } {
  ...
  if { [llength [sproc_build_info -build_label $build_label -cmd get]] == 0 } {
    puts "Error: No tasks found with the required reports."
    exit
  }
...
proc sproc_create_final_report {} {
  ...
  if { [llength [sproc_build_info -build_label $build_label -cmd get]] == 0 } {
    set NoInfo 1
    set NoInfoMessage "No available reports"
  } elseif { [llength [sproc_metric -cmd metrics]] == 0 } {
    set NoInfo 1
    set NoInfoMessage "No available metrics"
  } else {
    set NoInfo 0
  }
