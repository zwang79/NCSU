...
// Data hazard detector. Check if there is data dependency that may result in hazard.
bool sim_pipe::DATA_hazard_detect() {
	opcode_t id_op = Ins[ID].opcode;
	opcode_t ex_op = Ins[EXE].opcode;
	opcode_t mem_op = Ins[WB].opcode;
	if (ex_op != SW) { // SW instruction is different, because Ins[ID].dest is not actually a destination operand.
		if (id_op != SW) {
