...
class sim_pipe_fp{
    ...
    // hazard related
        // structual
        bool exe_structural_hazard;
        unsigned mem_structual_stalls;
        bool mem_structual_hazard;
	...
	//operands stored in the exe unit temporarily due to latency
	unit_tmp_t exec_units_tmp[MAX_UNITS];
...
public:
	...
	*/
	...
	// RAW hazard detector
	bool RAW_detect(instruction_t Ins);

	// WAW hazard detector
	bool WAW_detect(instruction_t Ins);

	// branch op detector
	bool branch_detect(opcode_t opcode);

    // reset sp registers
	void reset_sp_registers(stage_t s);
...
private:
	...
	unsigned get_exe_unit_type(opcode_t opcode);
